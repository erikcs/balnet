...
)
{
  ...
  if (is.null(lambda)) {
    ...
    intercepts <- (lamlist$frac * intercepts[lamlist$left]) + ((1 - lamlist$frac) * intercepts[lamlist$right])
  }
...
)
{
  ...
  betas <- coefs[["betas"]]
  eta <- sp_tcrossprod_plus(newdata, betas, intercepts, object[["n_threads"]])
