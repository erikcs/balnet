// ------------------------------------------------------------
// Modified adelie_core header for CBPS GLM fitting
//
...
)
{
    ...
    */
    ...
    const auto p = X.cols();
    vec_value_t grad(p);
    X.mul(resid, vec_value_t::Ones(n), grad);
...
)
{
    ...
    while (1) {
        ...
        * balnet NOTE:
        *
        *  a) Default adelie GLM tolerance scaling is tol' = tol * (loss_null - loss_full) / hess_sum
        *     For CBPS calibration loss, deviance can be negative, so we use plain curvature-scaled CD instead
        *     by defining loss_null=1 and loss_full=0 (these quantities are not used anywhere else and are not relevant for cbps)
        *
        *  b) For certain data configurations (condition S2 in Tan (2020, appendix) the calibration loss diverges at lambda --> 0.
        *     This happens towards the end of the lambda path, and we try a heuristic to terminiate the path faster instead
        *     of waiting for the solver to declare non-convergance at the usual tolerance and max iterations.
        *     This raises an adelie CD exception, which is expected and handled in the wrapper.
        */
        ...
        state_gaussian_pin_naive_t state_gaussian_pin_naive(
            ...
            intercept, max_active_size,
            max_iters_prime, // NOTE: updated for balnet
